import {singleGateReply,firstPlan,intakeStatus,collectFirstReview,deliverFirstReview,executionDiff} from './single-gate.js';
import {firstRunDocument,singleGateWorkspace} from './single-gate-ui.js';
import {BRIEF_FIELDS,briefJournal,recordBrief,captureBrief,reviewPacket} from './brief.js';
import {demoReview,validateReview,currentReview} from './review-assistant.js';
import {briefRail,briefDocument,readableStrategy,reviewWorkspace} from './experience-ui.js';
...
for(const t of workspace.threads||[])if(t.reviewAssist?.status==='loading'){t.reviewAssist.status='error';t.reviewAssist.error='이전 초안 생성이 중단되었습니다. 다시 시도해 주세요.';}
const icons = {
...
const reviewBusy=new Map(),pendingReviews=new Map();let reviewSaveTimer;
...
const states = {draft:'대화 중',requested:'검토 대기',reviewed:'실행안 도착',preparing:'실행 준비'};
...
function board(t,version,actions=true){return firstRunDocument(t,version,actions,icon);}
...
function workActions(t){
 if(t.status==='requested')return `<div class="dock-actions"><button class="dock-result" data-action="show-result" data-id="${t.id}">${icon('file')} 요청한 실행안 ${icon('arrow')}</button><a class="dock-review" href="#/studio/${t.id}">마케터 검토 체험 ${icon('external')}</a></div><p class="dock-disclaimer">로컬 체험 · 실제 전달·결제 없음</p>`;
 if(['reviewed','preparing'].includes(t.status))return `<div class="dock-actions"><button class="dock-result" data-action="show-result" data-id="${t.id}">${icon('file')} 검토된 실행안과 준비 도구 ${icon('arrow')}</button></div><p class="dock-disclaimer">검토본 도착 · 게시·발송·집행은 별도 승인</p>`;
 return latestPlan(t)?`<div class="dock-actions"><button class="dock-result" data-action="show-result" data-id="${t.id}">${icon('file')} 첫 실행안 보기 <span>v${t.versions.length}</span>${icon('arrow')}</button></div>`:'';
}
function chatView(t) {
 ...
 const text=t.status==='requested'?'검토를 기다리고 있어요.':t.status==='reviewed'?'첫 실행안이 도착했어요. 필요한 준비물을 확인해 보세요.':t.status==='preparing'?'승인한 전략으로 실행을 준비해요.':last?.text||'';
 return `${header()}<aside class="brief-floating">${briefRail(t)}</aside><main id="main" class="chat-page conversation-dock"><div class="dock-top"><a href="#/" aria-label="새 의뢰">${icon('plus')}</a><span class="dock-user" title="${esc(user?.text||'')}">${esc((briefJournal(t).items.find(n=>n.key==='business')?.value||t.title).slice(0,55))}</span><button data-action="history" data-id="${t.id}" class="dock-history">대화 기록 ${icon('clock')}</button></div><div class="conversation" aria-live="polite" aria-relevant="additions">${waiting?`<div class="thinking"><span class="thinking-dots"><i></i><i></i><i></i></span>${config.mode==='live'?'답변 작성 중':'예시 응답 작성 중'}</div>`:stopped?`<div class="chat-error" role="alert"><span>${esc(t.error||'응답이 중단됐어요.')}</span><button data-action="retry" data-id="${t.id}" class="text-button">다시 시도</button></div>`:`<div class="dock-reply"><span class="dock-author">${last?.mode==='demo'?'예시 응답':'ZEDER'}</span><div class="message-text">${esc(text)}</div></div>${last?.quickReplies?.length&&t.status==='draft'?`<div class="quick-replies">${last.quickReplies.map(q=>`<button data-action="quick" data-id="${t.id}" data-value="${esc(q)}">${esc(q)}</button>`).join('')}</div>`:''}${workActions(t)}`}</div><div class="sticky-composer">${composer(t)}<p class="composer-note">${config.mode==='live'?'OpenAI API 전송 · ':''}이 브라우저에만 저장</p></div></main>`;
}
function showResult(t){
 if(!t||!latestPlan(t))return;
 const v=t.status==='preparing'?t.approvedVersion:t.status==='reviewed'?t.reviewedVersion:t.status==='requested'?t.submittedVersion:t.versions.length;
 const before=t.status==='requested'?`<div class="reader-status"><strong>마케터 검토 대기</strong><span>로컬 체험 · 실제 전달 없음</span><a href="#/studio/${t.id}">마케터 리포트 열기 ↗</a></div>`:'';
 openModal(t.status==='draft'?'검토할 첫 실행안':t.status==='requested'?'요청한 실행안':'전문가 검토본 · 로컬 체험',before+board(t,v,t.status==='draft'));dialog.classList.add('result-dialog');
}
...
function studioList() {
  ...
  return studioShell(`<div class="studio-page-title"><div><h1>전략 요청함</h1></div><span class="studio-demo">로컬 요청</span></div><div class="studio-stats">${[['검토할 요청','requested'],['전달 완료','reviewed'],['실행 준비','preparing']].map(([name,s])=>`<div><span>${name}</span><strong>${all.filter(t=>t.status===s).length}<small>건</small></strong></div>`).join('')}</div><div class="studio-inbox"><div class="inbox-head"><h2>전략 요청</h2><div class="filter-tabs">${[['all','전체'],['requested','검토 대기'],['reviewed','전달 완료']].map(([value,label])=>`<button class="${studioFilter===value?'active':''}" data-action="filter" data-value="${value}">${label}</button>`).join('')}</div></div>${items.length?items.map(t=>{const p=versionPlan(t,t.submittedVersion);return `<a class="studio-request" href="#/studio/${t.id}"><div class="studio-request-top">${badge(t)}<small>${time(t.updatedAt)}</small></div><h3>${esc(p?.title??t.title)}</h3><p>${esc(p?.business??t.title)}</p><div class="studio-request-bottom"><span>${icon('person')}${esc(p?.audience)}</span><span>${icon('wallet')}${esc(p?.budget)}</span><b>${t.status==='requested'?'전략 검토하기':'요청 확인하기'} ${icon('arrow')}</b></div></a>`;}).join(''):`<div class="empty-state">${icon('inbox')}<h2>검토할 요청이 없어요.</h2><a class="secondary" href="#/">새 요청 ${icon('arrow')}</a></div>`}</div>`);
}
...
function studioDetail(t){if(!versionPlan(t,t.submittedVersion))return studioList();return singleGateWorkspace(t,{icon,shell:studioShell});}
function render() {
  ...
  if(r.view==='studio'&&t?.status==='requested')queueMicrotask(()=>ensureReview(t));
...
async function runReply(t) {
  ...
  try {
    ...
    if(config.mode==='live') {
      ...
      const response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json','x-preview-key':key},body:JSON.stringify({generationId,messages,plan:latestPlan(t),notes:briefJournal(t,{includeProposal:false}).items.map(({key,value,status})=>({key,value,status}))}),signal:controller.signal});
      ...
      result={...singleGateReply(t),mode:'demo',metadata:{generationId,model:'demo-rules',createdAt:new Date().toISOString()}};
    }
    ...
    if(result.plan){for(const n of briefJournal(t,{includeProposal:false}).items)if(['business','audience','goal','budget'].includes(n.key)&&n.source==='confirmed')result.plan[n.key]=n.value;}
...
function collectReview(form,t) {
  const data=new FormData(form); const p=clone(t.reviewDraft?.base===Number(form.dataset.base)?t.reviewDraft.plan:versionPlan(t,Number(form.dataset.base)));
...
function collectReviewLoose(form,t){
 const data=new FormData(form),base=Number(form.dataset.base),p=clone(t.reviewDraft?.base===base?t.reviewDraft.plan:versionPlan(t,base));
 for(const key of ['title','audience','goal','budget','summary','hypothesis'])p[key]=String(data.get(key)??'');
 p.paths=p.paths.map((path,i)=>({...path,...Object.fromEntries(['title','action','reason','signal'].map(k=>[k,String(data.get(`path-${i}-${k}`)??'')])),tools:data.getAll(`path-${i}-tools`)}));
 return {plan:p,note:String(data.get('review-note')??''),base};
}
function ensureReview(t){
 if(route().view!=='studio'||route().id!==t.id||reviewBusy.has(t.id)||t.status!=='requested')return;
 const packet=reviewPacket(t);if(currentReview(t,packet)||t.reviewAssist?.contextKey===packet.contextKey&&['error','cancelled','loading'].includes(t.reviewAssist.status))return;
 if(config.mode==='live'&&!sessionStorage.getItem('zeder-preview-access'))return;
 generateReviewDraft(t);
}
function applyReviewDraft(t,a){
 if(t.status!=='requested'||a.base!==t.submittedVersion)throw new Error('최신 요청을 다시 확인해 주세요.');
 t.reviewDraftHistory??=[];if(t.reviewDraft)t.reviewDraftHistory.push({...clone(t.reviewDraft),at:new Date().toISOString()});
 t.reviewDraft={base:a.base,plan:clone(a.review.plan),note:a.review.reply,dirty:false,assistantId:a.id};persist();render();
}
async function generateReviewDraft(t,force=false){
 if(!t||t.status!=='requested'||reviewBusy.has(t.id))return;
 if(config.mode==='live'&&!sessionStorage.getItem('zeder-preview-access')){settings();return;}
 const packet=reviewPacket(t),controller=new AbortController(),id=uid();reviewBusy.set(t.id,controller);
 t.reviewAssist={...t.reviewAssist,contextKey:packet.contextKey,status:'loading',error:null,versions:t.reviewAssist?.versions||[]};render();
 try{
  let result;
  if(config.mode==='live'){
   const response=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json','x-preview-key':sessionStorage.getItem('zeder-preview-access')||''},body:JSON.stringify({intent:'marketer_review',generationId:id,...packet}),signal:controller.signal});
   const data=await response.json();if(!response.ok)throw new Error(data.error||'검토 초안을 생성하지 못했어요.');
   if(data.metadata?.contextKey!==packet.contextKey||data.metadata?.requestVersion!==packet.requestVersion||data.metadata?.generationId!==id)throw new Error('다른 요청의 응답입니다. 다시 시도해 주세요.');result=data;
  }else{result={review:demoReview(packet),mode:'demo',metadata:{generationId:id,contextKey:packet.contextKey,requestVersion:packet.requestVersion}};}
  if(controller.signal.aborted||reviewBusy.get(t.id)!==controller)throw new DOMException('Aborted','AbortError');
  if(t.status!=='requested'||reviewPacket(t).contextKey!==packet.contextKey)throw new Error('고객 요청이 변경됐습니다. 최신 의뢰를 다시 열어 주세요.');
  const a={id,base:packet.requestVersion,contextKey:packet.contextKey,review:validateReview(result.review,packet),mode:result.mode,metadata:result.metadata,createdAt:new Date().toISOString()};
  t.reviewAssist.versions.push(a);t.reviewAssist.activeId=id;t.reviewAssist.status='ready';
  // Never overwrite a marketer's unsent edits when a delayed AI reply arrives.
  // Separate AI notes from the marketer's original editable report; no automatic replacement.
 }catch(e){t.reviewAssist.status=e.name==='AbortError'?'cancelled':'error';t.reviewAssist.error=e.name==='AbortError'?'초안 생성을 중지했습니다.':e.message;}
 finally{reviewBusy.delete(t.id);persist();if(route().view==='studio'&&route().id===t.id)render();}
}
document.addEventListener('submit',async e=>{
  ...
  try {
    ...
    if(f.dataset.form==='single-review'){
      const t=thread(f.dataset.id),value=collectFirstReview(f,t);deliverFirstReview(t,value);persist();render();toast('첫 실행안 전달 완료 · 로컬 체험');
    }
    if(f.dataset.form==='brief-edit'){const t=thread(f.dataset.id);recordBrief(t,f.dataset.key,f.elements.value.value);persist();render();openModal('의뢰 노트',briefDocument(t));dialog.classList.add('brief-dialog');toast('의뢰 노트에 기록했어요. 기존 전략 버전은 유지됩니다.');}
    if(f.dataset.form==='revision'){const t=thread(f.dataset.id);requestRevision(t,f.elements.note.value);captureBrief(t);t.reviewAssist=null;t.reviewDraft=null;persist();dialog.close();render();toast('수정 요청 저장됨');}
  }catch(error){toast(error.message);}
});
document.addEventListener('input',e=>{
  if(e.target.closest('form[data-form=single-review]')){
   const f=e.target.closest('form'),t=thread(f.dataset.id);
   try{t.reviewDraft=collectFirstReview(f,t,true);clearTimeout(reviewSaveTimer);reviewSaveTimer=setTimeout(()=>{if(persist()){const el=$('#review-save-status');if(el)el.textContent='이 브라우저에 임시 저장됨';}},350);const el=$('#review-save-status');if(el)el.textContent='저장 중…';}catch(error){toast(error.message);}
  }
...
document.addEventListener('click',async e=>{
  ...
  try {
    if(action==='brief'){openModal('의뢰 노트',briefDocument(t));dialog.classList.add('brief-dialog');}
    if(action==='edit-brief'){const key=el.dataset.key;if(!BRIEF_FIELDS[key])return;const n=briefJournal(t).items.find(n=>n.key===key);openModal(BRIEF_FIELDS[key],`<form data-form="brief-edit" data-id="${t.id}" data-key="${key}"><label class="field"><span>${BRIEF_FIELDS[key]}</span><textarea name="value" rows="4" maxlength="1500" required>${esc(n?.value||'')}</textarea></label><p class="mini-note">이전 내용은 기록에 남고, 이미 요청한 전략은 바뀌지 않습니다.</p><button class="primary" type="submit">노트에 저장</button></form>`);}
    if(action==='show-version'){const v=Number(el.dataset.version);if(versionPlan(t,v)){openModal(`전략 v${v}`,board(t,v),'<button class="secondary" data-action="close-modal">공방으로 돌아가기</button>');dialog.classList.add('result-dialog');}}
    if(action==='review-history'){const packet=reviewPacket(t);openModal('제출 당시 고객 대화',packet.messages.map(m=>`<article class="source-transcript"><span>${m.role==='user'?'고객':'ZEDER'}</span><p>${esc(m.text)}</p></article>`).join(''));dialog.classList.add('history-dialog');}
    if(action==='generate-review')await generateReviewDraft(t,true);
    if(action==='cancel-review-ai'){reviewBusy.get(id)?.abort();}
    if(action==='apply-review-ai'){const a=currentReview(t,reviewPacket(t));if(a){if(t.reviewDraft?.dirty){openModal('편집 중인 초안에 반영할까요?','<p>지금 편집한 초안은 이전 편집 기록에 보관됩니다. 생성한 답변과 전략으로 편집기를 바꿉니다.</p>',`<button class="secondary" data-action="close-modal">취소</button><button class="primary" data-action="confirm-apply-review" data-id="${t.id}">초안 반영</button>`);}else applyReviewDraft(t,a);}}
    if(action==='confirm-apply-review'){const a=currentReview(t,reviewPacket(t));if(a){dialog.close();applyReviewDraft(t,a);}}
    if(action==='use-review-question'){const f=$('form[data-form=review]');if(f){f.elements['review-note'].value=(f.elements['review-note'].value+'\n'+el.dataset.value).trim();f.elements['review-note'].dispatchEvent(new Event('input',{bubbles:true}));f.elements['review-note'].focus();}}
    if(action==='confirm-review-send'){const v=pendingReviews.get(id);if(!v)throw new Error('미리보기를 다시 열어 주세요.');reviewRequest(t,v.plan,v.note,v.base);t.reviewDraft=null;pendingReviews.delete(id);persist();dialog.close();render();toast('검토본 전달됨 · 로컬 체험');}
    ...
    if(action==='quick'){if(dialog.open)dialog.close();await send(el.dataset.value,id);}
    if(action==='retry')await runReply(t);
    if(action==='stop')busy.get(id)?.abort();
    if(action==='revise-chat'){if(dialog.open)dialog.close();drafts.set(id,'');go(`chat/${id}`);setTimeout(()=>{$('#message')?.focus();$('#message')?.setAttribute('placeholder','어떤 부분을 바꿀까요?');},50);}
    ...
    if(action==='request'){
      if(busy.has(id))throw new Error('응답이 끝난 뒤 실행안을 확인해 주세요.');
      const intake=intakeStatus(t);
      if(!intake.ready){dialog.close();go(`chat/${id}`);toast('제품·서비스와 이번 목표부터 정리해 주세요. 고객과 예산은 미정이어도 됩니다.');}
      else{submitRequest(t);captureBrief(t);t.reviewAssist=null;t.reviewDraft=null;persist();dialog.close();go(`request/${id}`);toast('검토 요청 저장됨 · 실제 마케터 전달 없음');}
    }
    ...
    if(action==='save-review'){const f=$('form[data-form=single-review]');if(f){t.reviewDraft=collectFirstReview(f,t,true);persist();toast('임시 저장됨 · 미전달');}}
    if(action==='artifact'){
      const toolId=el.dataset.tool;
      if(!['reviewed','preparing'].includes(t.status))throw new Error('검토된 실행안이 필요해요.');
      const v=t.approvedVersion??t.reviewedVersion,p=versionPlan(t,v);
      if(!p.paths[0].tools.includes(toolId))throw new Error('이 실행안에 포함되지 않은 도구입니다.');
      let a=t.artifacts.find(a=>a.toolId===toolId&&a.version===v);
      if(!a){a={id:uid(),toolId,version:v,text:artifactFor(p,toolId),createdAt:new Date().toISOString()};t.artifacts.push(a);persist();render();}
      openModal(TOOLS[toolId].output,`<p class="mini-note">템플릿 초안 · 실제 조사·게시·발송 없음</p><pre class="artifact-preview">${esc(a.text)}</pre>`,`<button class="secondary" data-action="show-result" data-id="${id}">실행안으로 돌아가기</button><button class="primary" data-action="download-artifact" data-id="${id}" data-tool="${toolId}">파일로 받기 ${icon('download')}</button>`);
    }
    if(action==='download-artifact'){const a=t.artifacts.find(a=>a.toolId===el.dataset.tool&&a.version===(t.approvedVersion??t.reviewedVersion));if(a)download(`zeder-${a.toolId}-v${a.version}.md`,a.text);}
...
window.addEventListener('hashchange',()=>{if(dialog.open)dialog.close();render();window.scrollTo(0,0);});
...
document.addEventListener('invalid',e=>{let el=e.target.parentElement;while(el){if(el.tagName==='DETAILS')el.open=true;el=el.parentElement;}},true);

document.addEventListener('change',e=>{
 const id=e.target.dataset.selectPath;if(!id)return;const t=thread(id);if(!t||t.status!=='requested')return;
 const packet=reviewPacket(t),index=Number(e.target.value);if(!Number.isInteger(index)||!packet.plan.paths[index])return;
 const note=t.reviewDraft?.note||'';t.reviewDraft={base:packet.requestVersion,plan:firstPlan(packet.plan,index),note,dirty:true};persist();render();
});