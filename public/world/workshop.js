...
export function createWorkshop(scene,m){
 ...
 shell.resultAnchor=resultAnchor;
 const props=createWorkProps(shell,worker,result,ribbon);
 let night=0;
 return {...shell,worker,resultAnchor,interactive:[result,ribbon],
  ...
  update(dt,time,{reduce=false}={}){shell.update(dt,time);pose=director.update(dt,{reduce});worker.root.position.set(...pose.position);worker.root.rotation.y=pose.yaw;const contacts=props.update(dt,time,pose,reduce);worker.update(dt,time,pose,contacts,reduce);result.visible=ribbon.visible=pose.resultAvailable;for(const {light,base}of lamps)light.intensity=base*(1+night*1.15)*(1+Math.sin(time*1.9)*.016);},
  debug(){return {...shell.debug(),worker:worker.root.position.toArray(),pose:'contact-rig-v2',night,activity:director.view(),maker:worker.debug(),objects:props.debug()};}
 };
}
