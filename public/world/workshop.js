...
import {createMaker} from './rigged-maker.js';
...
export function createWorkshop(scene,m){
 ...
 const worker=createMaker(shell.root);worker.root.scale.setScalar(1.25);
 ...
 const resultAnchor=new T.Object3D();resultAnchor.position.set(1.1,2.15,1.36);shell.root.add(resultAnchor);shell.resultAnchor=resultAnchor;
 const props=createWorkProps(shell,worker,result,ribbon);let night=0;
 return {...shell,worker,resultAnchor,interactive:[result,ribbon],
  setState(info){shell.setState(info);if(draftSheets)draftSheets.visible=!!info.outputId;director.observe(info);pose=director.view();props.setState(info);result.visible=ribbon.visible=pose.resultAvailable;},
  setNight(value){night=Math.max(0,Math.min(1,value));for(const mat of glows)mat.emissiveIntensity=1.2+night*1.4;},
  update(dt,time,{reduce=false}={}){shell.update(dt,time);pose=director.update(dt,{reduce});worker.root.position.set(...pose.position);worker.root.rotation.y=pose.yaw;worker.update(dt,time,pose,{},reduce);props.update(dt,time,pose,reduce);result.visible=ribbon.visible=pose.resultAvailable;for(const {light,base}of lamps)light.intensity=base*(1+night*1.15)*(1+Math.sin(time*1.9)*.016);},
  debug(){return {...shell.debug(),worker:worker.root.position.toArray(),pose:'skinned-maker-v3',night,activity:director.view(),maker:worker.debug(),objects:props.debug()};}
 };
}
