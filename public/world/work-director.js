// Semantic request state and visual choreography are intentionally separate.
// A timer can finish a walk, NEVER a request, review, publication or research job.
export const STATIONS=Object.freeze({
 desk:{position:[-1.24,.405,3.08],yaw:Math.PI+.08},
 board:{position:[1.15,.405,-2.12],yaw:Math.PI},
 shelf:{position:[-2.38,.405,-1.86],yaw:Math.PI},
});
...
export function taskFromInput(text='',question=''){
 ...
 if(/취소|중지/.test(input))return 'brief'; // Only the Stop action cancels a request.
 ...
 if(/고객|타깃|타겟|누구/.test(q))return 'audience';
 if(/예산|전략/.test(q))return 'strategy';
 return 'brief';
}
...
function semantic(s){
 if(s.stage==='error')return s.cancelled?'cancelled':'error';
 if(s.stage==='thinking')return 'processing';
 if(s.stage==='requested')return 'review_wait';
 if(s.stage==='reviewed')return 'review_ready';
 if(s.stage==='preparing')return 'approved';
 return s.outputId?'result_ready':s.threadId?'awaiting_input':'idle';
}
// A clear aisle around the real desk: never interpolate directly through it.
export function routeBetween(from,to){
 const start=[...from],end=[...to],points=[];
 const front=p=>p[2]>2.4,back=p=>p[2]<.15;
 if((front(start)&&back(end))||(back(start)&&front(end))){
  points.push([-3.86,.405,start[2]],[-3.86,.405,end[2]]);
 }
 points.push(end);return points.filter((p,i)=>distance(i?points[i-1]:start,p)>.02);
}
export function createWorkDirector(){
 let ambientRound=0;
 let info={},state='idle',action='idle',task='brief',generation=null,station='desk',elapsed=0;
 ...
 function observe(nextInfo){
  ...
  if(switched){queue=[];current=null;resetPose();generation=null;}
  task=nextInfo.task||'brief';
  if(newGeneration){generation=nextInfo.generationId;record('request-start');const name=stationFor(task);
   choreograph([{action:'receive',duration:.55,station},...stepsTo(name),{action:name==='shelf'?'research':'think',duration:Infinity,station:name}]);return;
  }
  if(state==='error'||state==='cancelled'){
   if(action!==state){choreograph([{action:state,duration:Infinity}]);record('request-stopped');}return;
  }
  // Settled data loaded/revisited must not replay a fictitious new job.
  ...
  if(newOutput){record('result-available');const name=stationFor(task),to=STATIONS[name].position;
   // This is presentation of an ALREADY available result, not fake processing.
   choreograph([...stepsTo(name),{action:'read',duration:.55,station:name},...stepsTo('desk',true,to),{action:'prepare',duration:.55,station:'desk',carry:true},{action:'ready',duration:1.0,station:'desk'}]);return;
  }
  if(wasProcessing&&state!=='processing'&&oldResponse!==info.responseId){
   record('answer-available');const name=stationFor(task);
   choreograph([...stepsTo(name),{action:'read',duration:.8,station:name}]);return;
  }
  if(['review_wait','review_ready','approved'].includes(state)){
   if(state==='review_wait'&&action!=='wait'){choreograph([{action:'wait',duration:Infinity}]);record('review-wait');}
 ...
 function ambientSteps(){
  ...
  const steps=[...stepsTo('desk'),{action:'tidy',duration:3.6+(ambientRound++%3)*.35,station:'desk'},
   {action:ambientRound%2?'read':'stretch',duration:2.2,station:'desk'},{action:'look-sea',duration:3.0,station:'desk'},
 ...
 function update(dt,{reduce=false}={}){
  ...
  if(current.action==='walk'){
   const d=distance(position,current.to),desired=Math.atan2(current.to[0]-position[0],current.to[2]-position[2]);
   const error=angle(yaw,desired);yaw+=error*(1-Math.exp(-delta*8));
   const wanted=Math.abs(error)>.72?0:Math.min(1.85,Math.sqrt(d*5.2));
   speed+=(wanted-speed)*(1-Math.exp(-delta*6));const move=Math.min(d,delta*speed);
   if(d>.005){position[0]+=(current.to[0]-position[0])*move/d;position[2]+=(current.to[2]-position[2])*move/d;}
