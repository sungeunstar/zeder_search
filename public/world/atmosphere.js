...
vec3 skyColor(vec3 d){
 float elevation=max(d.y,0.);vec3 horizon=vec3(.60,.62,.61);vec3 zenith=vec3(.008,.085,.30);
 vec3 col=mix(horizon,zenith,pow(elevation,.18));
 vec3 sun=normalize(vec3(-.38,.19,-.92));float alignment=max(dot(d,sun),0.);
 col+=vec3(.42,.24,.11)*pow(alignment,8.)*.75;
 col=mix(col,vec3(5.2,3.75,1.80),pow(alignment,1800.));
 ...
 vec3 cloudColor=mix(vec3(.76,.81,.83),vec3(1.00,.77,.53),pow(alignment,6.));
 col=mix(col,cloudColor,clouds*.38);return col;
}`;
export function atmosphere(scene){
 ...
 vec3 deep=vec3(.009,.16,.245),shallow=vec3(.028,.32,.40);
 vec3 color=mix(mix(deep,shallow,fbm(p*.045)*.45),skyColor(reflection),clamp(fresnel*.42+.10,0.,1.));
 vec3 sun=normalize(vec3(-.38,.19,-.92));float spec=pow(max(dot(reflect(-sun,n),view),0.),180.);
 color+=vec3(4.5,3.1,1.45)*spec;
 vec2 shore=p-vec2(7.,-6.3);float a=atan(shore.y/10.5,shore.x/18.);float coast=1.+.055*sin(a*3.+.4)+.034*cos(a*7.-1.)+.019*sin(a*11.);
 float dist=(length(shore/vec2(18.,10.5))-coast-.105)*19.;
