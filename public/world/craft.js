...
export function texture(kind,seed=2){const n=256,c=document.createElement('canvas');c.width=c.height=n;const ctx=c.getContext('2d'),img=ctx.createImageData(n,n),rand=random(seed);for(let y=0;y<n;y++)for(let x=0;x<n;x++){let v;if(kind==='wood')v=.73+.09*Math.sin(x*.7+noise(x*.04,y*.015)*18)+.12*fbm(x*.2,y*.012)+rand()*.035;else if(kind==='bark')v=.38+.25*fbm(x*.075,y*.04)+.16*Math.sin(x*.14+noise(x*.04,y*.03)*8)+rand()*.08;else v=.35+.6*fbm(x*.075,y*.075)+rand()*.045;const i=(y*n+x)*4;img.data[i]=Math.min(255,v*255);img.data[i+1]=Math.min(255,v*247);img.data[i+2]=Math.min(255,v*230);img.data[i+3]=255;}ctx.putImageData(img,0,0);if(kind==='rock'){
 ctx.globalAlpha=.23;for(let row=-1;row<13;row++)for(let col=-1;col<13;col++){
 const xx=col*24+(row%2)*10,yy=row*23,w=14+rand()*13,h=15+rand()*14,grey=Math.floor(100+rand()*125);
 ctx.beginPath();ctx.moveTo(xx+rand()*5,yy);ctx.lineTo(xx+w*.8,yy-3);ctx.lineTo(xx+w+4,yy+h*.36);ctx.lineTo(xx+w*.72,yy+h);ctx.lineTo(xx+3,yy+h*.92);ctx.lineTo(xx-4,yy+h*.5);ctx.closePath();ctx.fillStyle=`rgb(${grey},${grey},${grey})`;ctx.fill();ctx.lineWidth=.5;ctx.strokeStyle='#212020';ctx.stroke();
 }ctx.globalAlpha=1;
 }if(kind==='wood'){ctx.strokeStyle='rgba(34,17,7,.2)';for(let i=0;i<70;i++){const x=rand()*n;ctx.beginPath();ctx.moveTo(x,0);for(let y=0;y<=n;y+=12)ctx.lineTo(x+Math.sin(y*.04+i)*1.5,y);ctx.lineWidth=rand()*.8+.2;ctx.stroke();}}
