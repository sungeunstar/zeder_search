...
const css=(await Promise.all(['styles.css','quiet.css'].map(file=>readFile(`public/${file}`,'utf8')))).join('\n');
...
const html=shell.replace(/<link rel="stylesheet"[^>]+>/g,'').replace('</head>','<style>'+css+'</style></head>').replace(/<script type="module"[^>]+><\/script>/,'').replace('</body>','<script type="module">'+aliases+bundle.replace(/<\/script/gi,'<\\/script')+'</script></body>');
