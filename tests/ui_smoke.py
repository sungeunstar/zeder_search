...
with sync_playwright() as p:
 ...
 expect(page.locator('.home-description, .how-it-works, .home-footer, .home-orbit')).to_have_count(0)
 expect(page.locator('.home-page > .eyebrow')).to_have_count(0)
 expect(page.locator('.composer-accessories > span')).to_have_count(0)
 expect(page.locator('.mode-button')).to_have_text('예시 모드')
 assert [x.inner_text() for x in page.locator('.starter-prompts button').all()] == ['첫 고객 찾기','브랜드 알리기','마케팅 점검']
 ok('Home has no explanatory copy, process tutorial, decorative badge or footer slogan')
 page.locator('[data-action=settings]').click()
 expect(page.locator('dialog')).to_contain_text('이 브라우저에만 저장')
 expect(page.locator('dialog')).to_contain_text('OpenAI API로 전송')
 expect(page.locator('dialog')).to_contain_text('고객 탐색·발송·결제는 미연결')
 page.locator('dialog [data-action=close-modal]').first.click()
 ok('Settings preserve privacy and unavailable-integration disclosures')
 ...
 expect(page.locator('.message-text').last).to_have_text('누구에게 먼저 알리고 싶으세요?')
 expect(page.locator('.message-author small')).to_have_text('예시 응답')
 ok('Demo reply asks the business question without interface explanation')
 ...
 expect(page.locator('.path-index > span, .path-tools > .micro-label')).to_have_count(0)
 expect(page.locator('.message-text').last).to_have_text('먼저 이 세 가지를 시도해 볼까요?')
 expect(page.locator('.goal-node strong')).to_contain_text('제품이 해결하는 문제')
 expect(page.locator('.path-action')).to_have_count(3)
 ok('Strategy keeps business content and tools while removing repeated explanatory labels')
 page.locator('.tool-chip').first.click()
 expect(page.locator('dialog')).to_be_visible()
 expect(page.locator('dialog')).to_contain_text('Runner 미연결')
 page.locator('dialog [data-action=close-modal]').first.click()
 ok('Tool detail shows output and integration state without UI tutorial')
 page.locator('[data-action=request]').click()
 assert state(page)['threads'][0]['status']=='draft'
 expect(page.locator('dialog')).to_contain_text('실제 전달·결제·광고 집행 없음')
 page.locator('[data-action=confirm-request]').click();wait(page)
 expect(page.locator('h1')).to_contain_text('검토 대기')
 ...
 expect(page.locator('.studio-sidebar-note, .editor-footer > span, .editor-fields > .field-hint')).to_have_count(0)
 expect(page.locator('.context-warning')).to_contain_text('미검증')
 ok('Studio removes coaching copy while keeping editable fields and unverified-data status')
 ...
 expect(page.locator('h1')).to_contain_text('검토본 확인')
